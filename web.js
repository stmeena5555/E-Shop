function HeaderComponent() {
    ...
    small.innerText = "Shop at single click";
    ...
    signUp.setAttribute("style", "cursor:pointer");
...
function SignUpComponent() {
    let main = document.getElementById("main");
    let existingForm = document.getElementById("signUpForm");

    if (existingForm) {
        main.removeChild(existingForm);
        return;
    }

    let formDiv = document.createElement("div");
    formDiv.setAttribute("id", "signUpForm");
    formDiv.setAttribute("style", "height:450px; width:500px; background: black; color:white; position: absolute; top: 100px; left: 50%; transform: translateX(-50%); padding: 20px; text-align: center; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2); border-radius: 10px;");

    let heading = document.createElement("h3");
    heading.innerText = "Sign Up";
    heading.setAttribute("style", "margin-bottom: 20px; font-size: 24px;");
    formDiv.appendChild(heading);

    addInputField(formDiv, "User Name", "text", "Enter User Name", "signup-name");
    addInputField(formDiv, "Email", "email", "Enter Email", "signup-email");
    addInputField(formDiv, "Password", "password", "Enter Password", "signup-password");

    let btn = document.createElement("button");
    btn.setAttribute("type", "button");
    btn.setAttribute("style", "margin-top: 30px; padding: 10px 20px; background-color:black; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; transition: background-color 0.3s ease;");
...
function SignInComponent() {
    ...
    let formDiv = document.createElement("div");
    formDiv.setAttribute("id", "signInForm");
    formDiv.setAttribute("style", "height:350px; width:400px; background:black; color:white; position: absolute; top: 100px; left: 50%; transform: translateX(-50%); padding: 20px; text-align: center; box-shadow: 0 4px 8px black; border-radius: 10px;");
    ...
    addInputField(formDiv, "Email", "email", "Enter Email", "signin-email");
    addInputField(formDiv, "Password", "password", "Enter Password", "signin-password");
...
function validateForm(event, formType) {
    ...
    if (isValid) {
        alert(formType === 'signup' ? 'Sign Up successful!' : 'Sign In successful!');
        if (formType === 'signup') {
...
function addInputField(container, labelText, inputType, placeholder, id) {
    let label = document.createElement("label");
    label.innerText = labelText;
    label.setAttribute("style", "display: block; text-align: left; font-weight: bold; margin-top: 10px; color:white;");
    ...
    input.setAttribute("style", "width: 100%; padding: 10px; margin: 5px 0; border: 1px solid white; border-radius: 5px; font-size: 14px; background-color: black; color: white;");
...
function ViewMoreComponent(product) {
    ...
    discountPercentage.innerHTML = "<b>Discount : </b> " + product.discountPercentage;
    productDetailsContainer.appendChild(discountPercentage);
...
buyNow.addEventListener("click", function () {
    ...
    if (userData) {
        ...
        let buyingForm = document.createElement("div");
        buyingForm.setAttribute("style", "padding: 20px; background: black; border: 1px solid #ccc; border-radius: 5px; box-shadow: 10px 10px 10px grey; color: white; margin: 200px; margin-top: 50px;");

        let heading = document.createElement("h3");
        heading.innerText = "Buy Now";
        buyingForm.appendChild(heading);

        let productDetails = document.createElement("div");
        productDetails.innerHTML = "<p><b>Product:</b> " + product.title + "</p>" +
                                   "<p><b>Price:</b> " + product.price + " Rs.</p>";
        buyingForm.appendChild(productDetails);

        let shippingAddressLabel = document.createElement("label");
        shippingAddressLabel.innerText = "Shipping Address:";
        buyingForm.appendChild(shippingAddressLabel);

        let shippingAddressInput = document.createElement("textarea");
        shippingAddressInput.setAttribute("placeholder", "Enter your shipping address");
        shippingAddressInput.setAttribute("style", "width: 100%; height: 100px; margin-bottom: 10px; background-color: black; color: white;");
        buyingForm.appendChild(shippingAddressInput);

        let paymentMethodLabel = document.createElement("label");
        paymentMethodLabel.innerText = "Payment Method:";
        buyingForm.appendChild(paymentMethodLabel);

        let paymentMethodSelect = document.createElement("select");
        paymentMethodSelect.setAttribute("style", "width: 100%; padding: 5px; margin-bottom: 10px;");
        paymentMethodSelect.innerHTML = "<option value='credit_card'>Credit Card</option>" +
                                       "<option value='upi'>UPI</option>" +
                                       "<option value='cod'>Cash on Delivery</option>";
        buyingForm.appendChild(paymentMethodSelect);

        let confirmPurchaseButton = document.createElement("button");
        confirmPurchaseButton.innerText = "Confirm Purchase";
        confirmPurchaseButton.setAttribute("style", "padding: 10px 20px; background-color: #28a745; color: white; border: none; border-radius: 5px; cursor: pointer;");
        ...
        buyingForm.appendChild(confirmPurchaseButton);

        main.appendChild(buyingForm);
    } else {
        alert("Please sign up to proceed with the purchase.");
        let main = document.getElementById("main");
        main.innerHTML = "";
        SignUpComponent();
    }
